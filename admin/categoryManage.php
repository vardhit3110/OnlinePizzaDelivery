    ...
    <div class="col-lg-12">
        <div class="row">
                        ...
                        <table class="table table-bordered table-hover mb-0">
                            <thead style="background-color: rgb(111 202 203);">
                                <tr>
                                    ...
                                    <th class="text-center" style="width:45%;">Category Detail</th>
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    $status = $row['status']; // Assuming 'status' column exists in the database
                                
                                    echo '<tr>
            <td class="text-center"><b>' . $catId . '</b></td>
            <td>
                <img src="/OnlinePizzaDelivery/img/card-' . $catId . '.jpg" 
                     alt="Category Image" width="150px" height="150px">
            </td>
            <td>
            ...
            <td class="text-center">
                <form method="POST" action="partials/_categoryManage.php">
                    <input type="hidden" name="catId" value="' . $catId . '">
                    <select name="status" class="form-control status-select" onchange="changeStatusColor(this); this.form.submit();">
            ...
            <td class="text-center">
                <div class="row mx-auto" style="width:112px">
                    <button class="btn btn-sm btn-primary edit_cat" 
                            type="button" data-toggle="modal" 
                            data-target="#updateCat' . $catId . '">Edit</button>
                        ...
                        <button name="removeCategory" class="btn btn-sm btn-danger" 
                                ...
                                while ($catRow = mysqli_fetch_assoc($catResult)) {
                                    ...
                                    <div class="modal fade" id="updateCat<?php echo $catId; ?>" tabindex="-1" role="dialog"
                                        aria-labelledby="updateCat<?php echo $catId; ?>" aria-hidden="true"
                                                        ...
                                                        <div class="text-left my-2 row"
                                                            ...
                                                            <div class="form-group col-md-8">
                                                                ...
                                                                <input type="file" name="catimage" id="catimage"
                                                                    accept=".jpg" class="form-control" required
                                                                    style="border:none;"
                                                                    onchange="document.getElementById('itemPhoto').src = window.URL.createObjectURL(this.files[0])">
                                                                <small id="Info" class="form-text text-muted mx-3">Please
                                                                    .jpg file upload.</small>
                                                                <input type="hidden" id="catId" name="catId"
                                                                    value="<?php echo $catId; ?>">
                                                                <button type="submit" class="btn btn-success my-1"
                                                                    name="updateCatPhoto">Update Img</button>
                                                            </div>
                                                            <div class="form-group col-md-4">
                                                                <img src="/OnlinePizzaDelivery/img/card-<?php echo $catId; ?>.jpg"
                                                                    id="itemPhoto" name="itemPhoto" alt="Category image"
                                                    ...
                                                    <form action="partials/_categoryManage.php" method="post">
                                                        <div class="text-left my-2">
                                                            ...
                                                            <input class="form-control" id="name" name="name"
                                                                value="<?php echo $catName; ?>" type="text" required>
                                                        </div>
                                                        <div class="text-left my-2">
                                                            <b><label for="desc">Description</label></b>
                                                            <textarea class="form-control" id="desc" name="desc" rows="2"
                                                                required minlength="6"><?php echo $catDesc; ?></textarea>
                                                        </div>
                                                        <input type="hidden" id="catId" name="catId"
                                                            value="<?php echo $catId; ?>">
                                ...
                                <script>
                                    document.addEventListener("DOMContentLoaded", function () {
                                        document.querySelectorAll("form[action='partials/_categoryManage.php']").forEach(form => {
                                            form.addEventListener("submit", function (event) {
                                                let name = form.querySelector("input[name='name']");
                                                let desc = form.querySelector("textarea[name='desc']");
                                    ...
                                    // Validation for modal update form
                                    document.querySelectorAll("form[action='partials/_categoryManage.php']").forEach(form => {
                                        form.addEventListener("submit", function (event) {
                                            let name = form.querySelector("input[name='name']");
                                            let desc = form.querySelector("textarea[name='desc']");

                                            if (name && /\s/.test(name.value)) {
                                                event.preventDefault();
                                                alert("Category Name cannot contain spaces!");
                                            }

                                            if (desc && /\s/.test(desc.value)) {
                                                event.preventDefault();
                                                alert("Category Description cannot contain spaces!");
                                            }
                                        });
                                    });
                                    ...
                                    // Apply the color change on page load
                                    document.addEventListener("DOMContentLoaded", function () {
